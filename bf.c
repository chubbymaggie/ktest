...
void
    bf_local_memcpy (BF_LONG * s1, const BF_LONG * s2, int n)
{
    BF_LONG *p1;
    const BF_LONG *p2;
    ...
    BF_set_key (int len, unsigned char *data)
{
    ...
    bf_local_memcpy (key_P, bf_init_P, BF_ROUNDS + 2);
    bf_local_memcpy (key_S, bf_init_S, 4 * 256);
    p = key_P;
    ...
    )
{
    ...
    do{ blowfish_main(); } while (i++ < BENCHMARK_LOOP);
    ...
    DbgPrint("[KTEST] BenchmarkBfCalulation Execution took : %lld (100ns units)\n", Delta.QuadPart);
    Delta.QuadPart /= 10000; /* Convert to milliseconds, may cause us to lose precision */
    DbgPrint("[KTEST] BenchmarkBfCalulation Execution took : %lld ms\n", Delta.QuadPart);
