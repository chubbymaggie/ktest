    ...
    )
{
    ...
    do{ aes_main(); } while (i++ < BENCHMARK_LOOP);
    ...
    DbgPrint("[KTEST] BenchmarkAesCalulation Execution took : %lld (100ns units)\n", Delta.QuadPart);
    Delta.QuadPart /= 10000; /* Convert to milliseconds, may cause us to lose precision */
    DbgPrint("[KTEST] BenchmarkAesCalulation Execution took : %lld ms\n", Delta.QuadPart);
