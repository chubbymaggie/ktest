...
/* get and decode motion vector and differential motion vector 
   for one prediction */
void
motion_vector (int *PMV, int *dmvector, int h_r_size, int v_r_size, int dmv, int mvscale, int full_pel_vector)
{
  int motion_code;
  int motion_residual;
    ...
    )
{
    ...
    do{ mpeg2_main(); } while (i++ < BENCHMARK_LOOP);
    ...
    DbgPrint("[KTEST] BenchmarkMpeg2Decode Execution took : %lld (100ns units)\n", Delta.QuadPart);
    Delta.QuadPart /= 10000; /* Convert to milliseconds, may cause us to lose precision */
    DbgPrint("[KTEST] BenchmarkMpeg2Decode Execution took : %lld ms\n", Delta.QuadPart);
