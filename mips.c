/* 
 * Part of CHStone, checkout http://www.ertl.jp/chstone/ for Copyrights and licenses.
 * Uglified by Shahriyar Jalayeri
 */
#include "stdafx.h"
#include "tests.h"
    ...
    )
{
    ...
    do{ mips_main(); } while (i++ < BENCHMARK_LOOP);
    ...
    DbgPrint("[KTEST] BenchmarkMipsEmulation Execution took : %lld (100ns units)\n", Delta.QuadPart);
    Delta.QuadPart /= 10000; /* Convert to milliseconds, may cause us to lose precision */
    DbgPrint("[KTEST] BenchmarkMipsEmulation Execution took : %lld ms\n", Delta.QuadPart);
