...
TEST_ENTRY TestEntries[] = { 
    {BenchmarkPrimeCalc,                "BenchmarkPrimeCalc",                   "Benchmark -- Finding primes."},
    {BenchmarkSha1Calulation,           "BenchmarkSha1Calulation",              "Benchmark -- SHA1."},
    {BenchmarkAesCalulation,            "BenchmarkAesCalulation",               "Benchmark -- AES enc/dec."},
    {BenchmarkMipsEmulation,            "BenchmarkMipsEmulation",               "Benchmark -- Simple MIPS emulation."},
    {BenchmarkBfCalulation,             "BenchmarkBfCalulation",                "Benchmark -- Blowfish enc."}, 
    {BenchmarkMpeg2Decode,              "BenchmarkMpeg2Decode",                 "Benchmark -- MPEG2 decode."}, 
    ...
    {CxxTokensTest,                     "CxxTokensTest",                        "No description available."},   /* Failed, needs internal hook or cmpsX instrumentation             */
    ...
    )
{
    ...
    if (Num >= 4) {
        ...
        DbgPrint("[KTEST] passed the CounterTest!\n");
                ...
                if (Buffer[21] == ' ') {
                    ...
                    if (Buffer[22] == 'A') {
                        ...
                        if (Buffer[23] == ';') {
                            ...
                            TEST_SUCCESS_DBG_BREAK();
                            DbgPrint("[KTEST] passed the CxxTokensTest!\n");
    ...
    )
{
    ...
    if (bits == 15) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] passed the FourIndependentBranchesTest!\n");
    ...
    )
{
    ...
    if (BufferLen >= 20 && Buffer[1] == '1' && Buffer[5] == '5' && Buffer[10] == 'A' && Buffer[19] == 'Z') {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] passed the CleanseTest!\n");
    ...
    )
{
    ...
    if (bits == 63) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] passed the FullCoverageSetTest!\n");
        ...
        if (BufferLen >= 12 && !memcmp(Buffer + 8, "ABCD", 4)) {
            ...
            if (BufferLen >= 14 && !memcmp(Buffer + 12, "XY", 2)) {
                ...
                if (BufferLen >= 16 && !memcmp(Buffer + 14, "KLM", 3)) {
                    ...
                    TEST_SUCCESS_DBG_BREAK();
                    DbgPrint("[KTEST] passed the MemcmpTest!\n");
    ...
    if (BufferLen > 0 && Buffer[0] == 'H') {
        ...
        if (BufferLen > 1 && Buffer[1] == 'i') {
            ...
            if (BufferLen > 2 && Buffer[2] == '!') {
                ...
                TEST_SUCCESS_DBG_BREAK();
                DbgPrint("[KTEST] passed the NullDerefTest!\n");
    ...
    )
{
    ...
    if (x > 1234567890 &&
        ...
        a == 4242) {
            ...
            TEST_SUCCESS_DBG_BREAK();
            DbgPrint("[KTEST] passed the CmpTest!\n");
    ...
    )
{
    ...
    if (x == 12345678) {
            ...
            TEST_SUCCESS_DBG_BREAK();
            DbgPrint("[KTEST] passed the SimpleCmpTest!\n");
    ...
    )
{
    if (EqualString(Buffer, BufferLen, "AAA") &&
        ...
        BufferLen >= 14 && Buffer[13] == 42) {
            ...
            TEST_SUCCESS_DBG_BREAK();
            DbgPrint("[KTEST] passed the StrcmpTest!\n");
        ...
        if (BufferLen >= 12 && strncmp(S + 8, "ABCD", 4) == 0) {
            if (BufferLen >= 14 && strncmp(S + 12, "XY", 2) == 0) {
                if (BufferLen >= 16 && strncmp(S + 14, "KLM", 3) == 0) {
                    ...
                    TEST_SUCCESS_DBG_BREAK();
                    DbgPrint("[KTEST] passed the StrncmpTest!\n");
    ...
    )
{
    if (BufferLen >= 4  && ShortSwitch(Buffer, 2)) {
            ...
            TEST_SUCCESS_DBG_BREAK();
            DbgPrint("[KTEST] passed the ShortSwitchTest!\n");
    ...
    )
{
    if (BufferLen >= 4  && LongSwitch(Buffer, BufferLen)) {
            ...
            TEST_SUCCESS_DBG_BREAK();
            DbgPrint("[KTEST] passed the LongSwitchTest!\n");
    ...
    )
{
    if (BufferLen >= 4  && LongSwitch(Buffer, BufferLen) &&
        ...
        BufferLen >= 14 && ShortSwitch(Buffer + 12, 2)) {
            ...
            TEST_SUCCESS_DBG_BREAK();
            DbgPrint("[KTEST] passed the SwitchTest!\n");
        ...
        if (Buffer[3] > 0x10) {
            ...
            if (Buffer[5] == 0x41) {
                ...
                if (Buffer[7] < 0x90) {
                    ...
                    TEST_SUCCESS_DBG_BREAK();
                    DbgPrint("[KTEST] FourDependentBranchesTest, final stage passed!\n");
    ...
    )
{
    ...
    if (abs(x) < 0 && y == 0xbaddcafe) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] AbsNegAndConstantTest passed; x=0x%x y=0x%x\n", x, y);
    ...
    )
{
    ...
    if (_abs64(x) < 0 && y == 0xbaddcafedeadbeefULL) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] AbsNegAndConstantTest64 passed; x=0x%x y=0x%x\n", x, y);
    ...
    )
{
    ...
    if (BufferLen >= 3 && strcmp(S, "012") == 0) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] passed the SimpleStrcmpTest!\n");
    ...
    )
{
    ...
    if (BufferLen >= 3 && strncmp(S, "012", 3) == 0) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] passed the SimpleStrncmpTest!\n");
    ...
    )
{
    ...
    if (_stricmp(S, "aBc") == 0) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] passed the SimpleStricmpTest!\n");
    ...
    )
{
    ...
    if (_strnicmp(S, "aBc", 3) == 0) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] passed the SimpleStrnicmpTest!\n");
    ...
    )
{
    ...
    if (wcscmp(S, L"012") == 0) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] passed the SimpleWcscmpTest!\n");
    ...
    )
{
    ...
    if (_wcsicmp(S, L"aBc") == 0) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] passed the SimpleWcsicmpTest!\n");
    ...
    )
{
    ...
    if (wcsncmp(S, L"aBc", 3) == 0) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] passed the SimpleWcsncmpTest!\n");
    ...
    )
{
    ...
    if (_wcsnicmp(S, L"aBc", 3) == 0) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] passed the SimpleWcsnicmpTest!\n");
    ...
    )
{
    ...
    if (BufferLen > 3 && RtlCompareMemory(S, "abc", 3) == 3) {
        ...
        TEST_SUCCESS_DBG_BREAK();
        DbgPrint("[KTEST] passed the SimpleRtlCompareMemoryTest!\n");
    ...
    )
{
    ...
    if (BufferLen > 3) {
        ...
        if (RtlCompareString(&Src, &Dst, TRUE) == 0) {
            ...
            TEST_SUCCESS_DBG_BREAK();
            DbgPrint("[KTEST] passed the SimpleRtlCompareStringTest!\n");
    ...
    )
{
    ...
    if (BufferLen > 3) {
        ...
        if (RtlEqualString(&Src, &Dst, TRUE) == TRUE) {
            ...
            TEST_SUCCESS_DBG_BREAK();
            DbgPrint("[KTEST] passed the SimpleRtlEqualStringTest!\n");
    ...
    )
{
    ...
    if (BufferLen > (3 * sizeof(WCHAR))) {
        ...
        if (RtlCompareUnicodeString(&Src, &Dst, TRUE) == 0) {
            ...
            TEST_SUCCESS_DBG_BREAK();
            DbgPrint("[KTEST] passed the SimpleRtlCompareUnicodeStringTest!\n");
    ...
    )
{
    ...
    if (BufferLen > (3 * sizeof(WCHAR))) {
        ...
        if (RtlEqualUnicodeString(&Src, &Dst, TRUE) == TRUE) {
            ...
            TEST_SUCCESS_DBG_BREAK();
            DbgPrint("[KTEST] passed the SimpleRtlEqualUnicodeStringTest!\n");
    ...
    )
{
    ...
    DbgPrint("[KTEST] BenchmarkPrimeCalc Execution took : %lld (100ns units)\n", Delta.QuadPart);
    Delta.QuadPart /= 10000; /* Convert to milliseconds, may lose precision */
    DbgPrint("[KTEST] BenchmarkPrimeCalc Execution took : %lld ms\n", Delta.QuadPart);
    ...
    )
{
...

    {
        ...
        DbgPrint("[KTEST] Execution took : %lld TICS\n", EXEC_MEASURE_DIFF());
    }
